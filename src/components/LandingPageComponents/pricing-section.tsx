...
const handleCheckout = async (planName: string, stripePriceId?: string | null) => {
  if (!stripePriceId) {
  ...
  const applicantPlans = [
    {
      ...
      price: "$0",
      description: "Always Free - No cost, No time limit. Unlimited Access for Job Seekers.",
      features: [
      ...
      audience: "Job seekers of all levels of experience, who want to actively apply to jobs.",
  ...
  const employerPlans = [
    ...
    },
    {
      ...
      audience: "Growing businesses with regular hiring needs and a focus on candidate quality.",
      popular: true,
    ...
    },
    {
      ...
      audience: "Large enterprises and organizations with high-volume hiring needs.",
      popular: false,
  ...
  return (
    <Box component="section" sx={{ width: "100%", py: 8, px: 2, display: "flex", flexDirection: "column", alignItems: "center", background: "linear-gradient(to bottom, #f9fafb, #ffffff)" }}>
      {/* Header */}
      <Box textAlign="center" mb={5} maxWidth="700px">
        <Typography variant="h4" fontWeight={700}>Flexible Plans for Every Need</Typography>
        <Typography variant="body1" sx={{ color: "rgba(60,60,60,0.7)", mt: 1.5 }}>
          Whether you’re a job seeker or an employer, Unswayed offers transparent, powerful plans to help you find the right match.
        </Typography>
      </Box>

      {/* Plan Toggle */}
      <Box display="flex" gap={1} bgcolor="#f1f5f9" p={0.5} borderRadius="10px" mb={6}>
        <Button onClick={() => setIsApplicant(true)} variant={isApplicant ? "contained" : "text"}
          sx={{ borderRadius: "8px", fontWeight: 600, fontSize: "14px", px: 3,
          backgroundColor: isApplicant ? "#00305B" : "transparent", color: isApplicant ? "#fff" : "#475569" }}>
          Applicant Plans
        </Button>
        <Button onClick={() => setIsApplicant(false)} variant={!isApplicant ? "contained" : "text"}
          sx={{ borderRadius: "8px", fontWeight: 600, fontSize: "14px", px: 3,
          backgroundColor: !isApplicant ? "#00305B" : "transparent", color: !isApplicant ? "#fff" : "#475569" }}>
          Employer Plans
        </Button>
      ...
      {/* Pricing Cards */}
      <Box display="flex" flexWrap="wrap" gap={3} maxWidth="1100px" justifyContent="center" width="100%">
        {plans.map((plan) => (
          <Box
            key={plan.name}
            sx={{
          ...
          >
            ...
            <Typography variant="h4" fontWeight={700} lineHeight={1}>{plan.price}</Typography>
            <Typography variant="body2" sx={{ color: plan.popular ? "rgba(255,255,255,0.85)" : "#6b7280" }}>{plan.description}</Typography>
            ...
            <Button
              ...
              sx={{
                ...
                "&:hover": { backgroundColor: plan.popular ? "#147c2cff" : "#189e33ff", color: "#fff" },
              }}
            >
              {loadingPlan === plan.name ? "Processing..." : plan.buttonText}
            </Button>

            <Typography variant="body2" fontWeight={600} mt={2}>What’s Included:</Typography>
            <Box component="ul" sx={{ listStyle: "none", p: 0, m: 0, display: "flex", flexDirection: "column", gap: 1.5 }}>
              {plan.features.map((feature) => (
                <Box key={feature} component="li" sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <Check strokeWidth={2} color={plan.popular ? "#fff" : "#189e33ff"} size={18} />
                  <Typography variant="body2" sx={{ color: plan.popular ? "rgba(255,255,255,0.9)" : "#374151" }}>{feature}</Typography>
                </Box>
              ))}
            </Box>

            {isApplicant && plan?.audience && (
              <Typography variant="body2" sx={{ mt: 2, fontStyle: "italic", color: plan.popular ? "rgba(255,255,255,0.85)" : "#555" }}>
      ...
      {!isApplicant && (
        <Box mt={6} maxWidth="800px" textAlign="center">
          <Typography variant="body2" sx={{ color: "#4b5563", mb: 1 }}>
            <strong>Free Trial:</strong> Employers who sign up for the Free Trial Plan get full
            access to the Standard Plan features for 30 days. No credit card is required to begin
            the trial. After 30 days, Employers must upgrade to a paid plan to continue using the
            platform.
          </Typography>
          <Typography variant="body2" sx={{ color: "#4b5563", mb: 1 }}>
            <strong>Billing:</strong> Monthly or annual billing options available. Annual subscribers
            receive a 10% discount.
          </Typography>
          <Typography variant="body2" sx={{ color: "#4b5563" }}>
            <strong>Cancellation:</strong> Users can cancel any time before the next billing cycle to
            avoid charges.
          </Typography>
        </Box>
      )}
